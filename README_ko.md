...
MPU-6500은 3축 자이로스코프, 3축 가속도계 및 Digital MotionProcessor®(DMP®)를 모두 작은 3x3x0.9mm 패키지에 결합한 6축 MotionTracking 장치입니다. 또한 직렬 버스 인터페이스의 트래픽을 낮추고 시스템 프로세서가 센서 데이터를 버스트적으로 읽은 다음 저전력 모드로 전환할 수 있도록 하여 전력 소비를 줄일 수 있는 512바이트 FIFO를 갖추고 있습니다. MPU-6500은 전용 I2C 센서 버스를 통해 외부 I2C 장치의 입력을 직접 받아들입니다. 6축 통합, 온칩 DMP 및 런타임 교정 펌웨어를 갖춘 MPU-6500을 통해 제조업체는 비용이 많이 들고 복잡한 선택, 검증 및 개별 장치의 시스템 수준 통합을 제거하여 소비자에게 최적의 모션 성능을 보장할 수 있습니다. MPU-6500은 또한 보조 I2C 포트에서 압력 센서와 같은 여러 비관성 디지털 센서와 인터페이스하도록 설계되었습니다. 자이로스코프의 프로그래밍 가능한 전체 범위는 ±250, ±500, ±1000 및 ±2000도입니다. /sec 및 0.01dps/√Hz에서 매우 낮은 속도 노이즈. 가속도계에는 ±2g, ±4g, ±8g 및 ±16g의 사용자 프로그래밍 가능 가속도계 전체 범위가 있습니다. 두 센서 모두 공장에서 교정된 초기 감도 덕분에 생산 라인 교정 요구 사항이 줄어듭니다. 기타 업계 최고의 기능으로는 온칩 16비트 ADC, 프로그래밍 가능한 디지털 필터, -40°C~85°C에서 1% 드리프트가 있는 정밀 클록, 내장된 온도 센서 및 프로그래밍 가능한 인터럽트. 이 장치는 I2C 및 SPI 직렬 인터페이스, 1.71~3.6V의 VDD 작동 범위, 1.71~3.6V의 별도 디지털 IO 공급 장치인 VDDIO를 갖추고 있습니다. 장치의 모든 레지스터와의 통신은 400kHz의 I2C 또는 SPI를 사용하여 수행됩니다. 1MHz에서. 더 빠른 통신이 필요한 응용 분야의 경우 20MHz에서 SPI를 사용하여 센서 및 인터럽트 레지스터를 읽을 수 있습니다. InvenSense는 웨이퍼 레벨 본딩을 통해 MEMS 웨이퍼를 동반 CMOS 전자 장치와 통합하는 특허를 받고 대량으로 입증된 CMOS-MEMS 제조 플랫폼을 활용하여 다음을 구현했습니다. 패키지 크기를 3x3x0.90mm(24핀 QFN)의 풋프린트와 두께로 줄여 매우 작지만 고성능 저비용 패키지를 제공합니다. 이 장치는 10,000g의 충격 신뢰성을 지원하여 높은 견고성을 제공합니다.
